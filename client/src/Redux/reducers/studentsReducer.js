...
function studentsReducer(state = {students : [], errors:{}}, action) {
    switch (action.type) {
        ...
        case DELETE_STUDENT:
            return { students: state.students.filter(student => student._id !== action.payload._id) };
            case UPDATE_STUDENT:
            return {  students: state.students.map(student => student._id === action.payload._id ? action.payload : student)};
